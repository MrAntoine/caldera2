...
private _query = format ["SELECT exp_level, exp_total, exp_perkPoints, exp_perks FROM players WHERE uid = '%1'", getPlayerUID _unit];

diag_log ("Requesting player experience data for user ID " + (getPlayerUID _unit) + " | Query (1): " + _query);

private _queryResult = [_query,2] call EXTDB3_fnc_async;
