		...
		if (isNull g_company) then
		{
			if ((_cfg select 11) != str(playerSide)) then
			{
				if ([true, _price] call AlysiaClient_fnc_atmFactionHandle) then {
					[format["Votre faction à reçu <t color='#8cff9b'>%1</t>BTC pour la mise en fourrière du véhicule.", ([_price] call AlysiaClient_fnc_numberText)], "buy"] call AlysiaClient_fnc_info;
				};
			} else {
			...
			[format["Votre entreprise à reçu <t color='#8cff9b'>%1</t>BTC pour la mise en fourrière du véhicule.", ([_price] call AlysiaClient_fnc_numberText)], "buy"] call AlysiaClient_fnc_info;
