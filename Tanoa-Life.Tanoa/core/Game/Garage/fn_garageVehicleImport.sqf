...
if (_importPrice > g_atm) exitWith {
	[format["Il vous faut <t color='#ff8c8c'>%1BTC</t> pour rapatrier ce véhicule.", [_importPrice] call AlysiaClient_fnc_numberText]] call AlysiaClient_fnc_error;
};

_vehicleName = lbText[2802, _sel];
_action =
[
	format["Voulez-rapatrier votre <t color='#FF8000'>%1</t> pour <t color='#8cff9b'>%2</t>BTC ?", _vehicleName, ([_importPrice] call AlysiaClient_fnc_numberText)],
	"Rapatriement",
	"Valider",
	"Annuler"
...
if (_action) then
{
	...
	[format["Vous avez rapatrié <t color='#FF8000'>%1</t> pour <t color='#8cff9b'>%2</t>BTC", _vehicleName, ([_importPrice] call AlysiaClient_fnc_numberText)], "buy"] call AlysiaClient_fnc_info;
