...
try {
	...
	_updateGUI = {
		...
		if (isClass _nextLevelConfig) then {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1/%2, Pts de compétences dispo: %3", life_currentExp - _currentLevelBottomExp, _currentLevelTopExp - _currentLevelBottomExp, life_currentPerkPoints];
			_ctrlProgress progressSetPosition _currentLevelProgress;
		} else {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1, Niveau max, Pts de compétences dispo: %2", life_currentExp - _currentLevelBottomExp, life_currentPerkPoints];
	...
	switch (_mode) do
	{
		case "onLoad":
		{
			...
			if (!life_perksInitialized) exitWith {
				hint "Vos compétences sont en cours de chargement, merci d'essayer plus tard";
			};
					...
					if (_hasParents) then {
						if (!_ownsPerk) then {
							if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) then {
								_ctrlBtnPurchase ctrlSetText "Points insuffisants";
								_ctrlBtnPurchase ctrlEnable false;
							} else {
								_ctrlBtnPurchase ctrlSetText "Acheter";
								_ctrlBtnPurchase ctrlEnable true;
							};
						} else {
							_ctrlBtnPurchase ctrlSetText "Déjà acheté";
							_ctrlBtnPurchase ctrlEnable false;
						};
					} else {
						_ctrlBtnPurchase ctrlSetText "Non achetable";
						_ctrlBtnPurchase ctrlEnable false;
					};
				} else {
					_ctrlBtnPurchase ctrlSetText "Nv sup. requis";
					_ctrlBtnPurchase ctrlEnable false;
				};
			} else {
				_ctrlBtnPurchase ctrlSetText "MAUVAISE FACTION";
				_ctrlBtnPurchase ctrlEnable false;
			};
		};
		...
		case "onButtonPurchaseClick":
		{
			...
			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) exitWith {
				hint "Vous n'avez pas assez de points de compétences pour acheter cette compétence!";
			};

			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) then {
				hint "Vous n'avez pas le niveau requis pour débloquer cette compétence!";
			};
			...
			_ctrlBtnPurchase ctrlSetText "Achat!";
			_ctrlBtnPurchase ctrlEnable false;
