...
if (!(_object getVariable ["destroyed", false])) exitWith {
	["Le transmetteur n'est pas saboté."] call AlysiaClient_fnc_error;
};
...
if (([_repair_item] call AlysiaClient_fnc_itemCount) < _repair_amount) exitWith
{
	[
		format
		[
			"Vous avez besoin de %1x <t color='#FF4000'>%2</t> pour réparer le transmetteur.",
			_repair_amount,
...
if (["Réparation du transmetteur", 10, _object, "", "AinvPknlMstpsnonWnonDnon_medic_1"] call AlysiaClient_fnc_showProgress) then
{
	if (!(_object getVariable ["destroyed", false])) exitWith {
		["Le transmetteur n'est pas saboté."] call AlysiaClient_fnc_error;
	};
	...
	if ([false, _repair_item, _repair_amount] call AlysiaClient_fnc_handleInv) then
	{
		[
			format
			[
				"Vous avez utilisé %1x <t color='#FF4000'>%2</t> pour réparer le transmetteur.",
				_repair_amount,
				[_repair_item] call AlysiaClient_fnc_itemGetName
			]
